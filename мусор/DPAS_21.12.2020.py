...
for x in range(n):
    print("Длина пролетного строения №", x + 1, ":", dlinaproleta[x],
...
if len(nprom) != 0:
    ...
    Vrigprom1 = round(Hrig1 * Lrigprom * Bfasrigprom - (Bfasrigprom * (2 * (0.5 * 0.4 * 1.2))), 2)  # объем ригеля
...
if len(nprom) != 0:
    ...
    for j in range(len(nprom)):
        Lrostvprom = (shagstoekprom * (nstoekprom - 1)) + Dstoiki + 2 * 1.2  # расчет длины ростверка промежуточных опор
